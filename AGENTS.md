...
- State Management: Zustand (only if strictly necessary)
- Icons: `@heroicons/react`
- Styling utilities: `clsx` and `tailwind-merge` (use a `cn` utility function for merging dynamic classes)
...
src/
  ...
  features/     — User interactions and actions (auth, forms, etc.)
  entities/     — Business domain objects with ui/, model/ segments (job, skill)
  shared/       — Project-agnostic UI primitives and utilities (ui/, lib/, assets/)
```

- **Import rule:** Code may only import from layers below it (app → pages → widgets → features → entities → shared). Never import from the same layer or above.
...
- **Keep files under 200 lines.** Split into focused modules if exceeded.
...
- **Icons:** Use `@heroicons/react` (e.g., `import { HomeIcon } from '@heroicons/react/24/outline'`). Do not use raw SVGs unless an icon is unavailable in the library.
- **Images:** Import images statically at the top of the file (e.g., `import heroImg from '@/shared/assets/hero.webp'`) so Vite handles bundling and hashing. Avoid using the `public/` folder for images unless they are dynamic or required for external meta tags.
...
- **Be Concise:** Minimize explanations. Provide the necessary code and a brief summary of what changed.
- **Think Before You Act:** Search the codebase first if you are unsure about a project-specific type, FSD boundary, or import path. Do not guess.
- **Refactoring:** If modifying a file pushes it over the 200-line limit, propose a refactoring plan before writing the code.
- **No Pleasantries:** Skip greetings and pleasantries in your responses.
